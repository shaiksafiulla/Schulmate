	...
	event.respondWith(
		caches.match(event.request)
			.then(function (response) {
				...
				return fetch(event.request)
					.catch(() => caches.match('/offline.html')); // Offline page fallback if network fails
			})
	);
	if (APP_DIAG) { console.log('Service Worker: Fetching ' + APP_NAME + '-' + APP_VER + ' files from Cache'); }
});
...
self.addEventListener('push', function (event) {
	...
	const options = {
		...
		//icon: '/schoolcoremob/app/icons/icon-72x72.png',
		//badge: '/schoolcoremob/app/icons/icon-72x72.png',
		//sound: '/schoolcoremob/app/icons/notifyringtone.mp3', 
		icon: '/app/icons/icon-72x72.png',
		badge: '/app/icons/icon-72x72.png',
		sound: '/app/icons/notifyringtone.mp3',
	...
	// Post a message to the client (your main page) to trigger UI change
	event.waitUntil(
		self.clients.matchAll({ includeUncontrolled: true, type: 'window' }).then(function (clients) {			
			// Send the message to all clients (pages) to trigger the div display and update the message count
			if (clients.length > 0) {
				clients.forEach(client => {
					client.postMessage({
						type: 'SHOW_DIV',
						data: notificationData // Includes message and unread count
					});
				});
			}
		})
...
self.addEventListener('notificationclick', function (event) {
	...
	var servicepath = self.location.pathname.split('/')[1];
	if (!servicepath) {
		servicepath = '';
	} else {
	...
	//for production
	var fullservicepath = self.location.origin + servicepath;
	//for local
	//var fullservicepath = self.location.origin;	
	//console.log(fullservicepath + '/Notification');
	event.notification.close();
	event.waitUntil(
		//clients.openWindow('/')  // Open your app or a specific URL  https://localhost:7114/Notification
		clients.openWindow(fullservicepath + '/Notification')
	);
});